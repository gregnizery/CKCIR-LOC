...
// Sur un filesystem en couches (overlay2, comme sur Render/Docker), le tout
// premier exec() d'un binaire depuis l'image peut echouer en ETXTBSY si deux
// requetes lancent Chromium presque simultanement (le noyau "copie" encore le
// binaire en interne). C'est transitoire : un nouvel essai juste apres
// reussit presque toujours.
async function launchWithRetry(options, attempts = 3) {
  for (let i = 1; i <= attempts; i += 1) {
    try {
      return await puppeteer.launch(options);
    } catch (err) {
      if (err.code !== 'ETXTBSY' || i === attempts) throw err;
...
// Lance puis ferme immediatement un navigateur : force le "copy-up" du
// binaire Chromium par le noyau une bonne fois pour toutes au demarrage du
// serveur, plutot que de laisser les premieres requetes utilisateur se
// disputer cette etape et risquer l'ETXTBSY decrit plus haut.
async function warmUpBrowser() {
  try {
    const browser = await getBrowser();
    await browser.close();
  } catch (err) {
    console.error('Avertissement : prechauffage Chromium echoue:', err.message);
  }
}
...
module.exports = { generateContractPdf, numeroContrat, warmUpBrowser };
